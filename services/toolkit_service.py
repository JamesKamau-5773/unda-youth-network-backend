...
def create_toolkit_item(data: dict, creator_id: int) -> InstitutionalToolkitItem:
    ...
    item = InstitutionalToolkitItem(title=title, content=content, attachments=attachments, created_by=creator_id)
    db.session.add(item)
    db.session.commit()
    return item
...
def list_toolkit_items(published_only: bool = False):
    ...
    if published_only:
        q = q.filter_by(published=True).order_by(InstitutionalToolkitItem.published_at.desc())
    else:
